...
const UpdateBlog = ()=>{
    ...
    const todosdata = useAppSelector((state)=>state.blogReducer)
    console.log("red todo",todosdata)
    const [formData, setFormData] = useState({
      ...
      useEffect(()=>{
        if(todosdata.id){
            const blogToChange = todosdata.todos.filter((todo)=>todo.id === todosdata.id)
            console.log("blogDataFromRedux",blogToChange)
      ...
      const updateData = async ()=>{
        try{
           const res = await axios.put("http://localhost:8080/todos/"+todosdata.id,formData)
    ...
    return(
           <div className='bg-container-data'>
                ...
                <h1 className='create-blog-head'>Update Blog</h1>
